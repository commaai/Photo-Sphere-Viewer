  ...
  var VideoPlugin = /*#__PURE__*/function (_AbstractPlugin) {
    ...
    _proto.__autorotate = function __autorotate() {
      ...
      if (autorotate.start === autorotate.end) {
        ...
        var progress = (currentTime - autorotate.start.time) / (autorotate.end.time - autorotate.start.time);
    ...
    _proto.__autorotateNext = function __autorotateNext(currentTime) {
      ...
      var workPoints = [keypoints[Math.max(0, k1 - 1)].position, keypoints[k1].position, keypoints[k2].position, keypoints[Math.min(l, k2 + 1)].position];
