      ...
      for (var face = 0; face < 6; face++) {
        for (var col = 0; col < panorama.nbTiles; col++) {
          for (var row = 0; row < panorama.nbTiles; row++) {
            ...
            // top-left
            var v0 = face * NB_VERTICES_BY_PLANE + row * this.prop.facesByTile * CUBE_SEGMENTS * NB_VERTICES_BY_FACE + col * this.prop.facesByTile * NB_VERTICES_BY_FACE;

            // bottom-left
            var v1 = v0 + CUBE_SEGMENTS * NB_VERTICES_BY_FACE * (this.prop.facesByTile - 1) + 1;

            // bottom-right
            var v2 = v1 + this.prop.facesByTile * NB_VERTICES_BY_FACE - 3;
            ...
            if (this.prop.facesByTile >= CUBE_SEGMENTS / 2) {
              // top-center
              var v4 = v0 + this.prop.facesByTile / 2 * NB_VERTICES_BY_FACE - 1;

              // bottom-center
              var v5 = v1 + this.prop.facesByTile / 2 * NB_VERTICES_BY_FACE - 3;

              // left-center
              var v6 = v0 + CUBE_SEGMENTS * NB_VERTICES_BY_FACE * (this.prop.facesByTile / 2 - 1) + 1;

              // right-center
              var v7 = v6 + this.prop.facesByTile * NB_VERTICES_BY_FACE - 3;
    ...
    _proto.__swapMaterial = function __swapMaterial(face, col, row, material) {
      ...
      for (var c = 0; c < this.prop.facesByTile; c++) {
        var _loop = function _loop(r) {
          ...
          var faceRow = row * _this7.prop.facesByTile + r;

          // first vertex for this face (6 vertices in total)
          var firstVertex = NB_VERTICES_BY_PLANE * face + 6 * (CUBE_SEGMENTS * faceRow + faceCol);
